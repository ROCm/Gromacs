...
static void setup_kernel_test(std::string dataDir, kernelTest &test, bool calcVir, bool calcEner)
{
    ...
    MY_CUDA_CHECK( cudaMalloc((void**)&test.kernParam.d_forceParams, sizeof(t_iparams) * test.numFFParamTypes) ); 
    MY_CUDA_CHECK( cudaMemcpy((void*)test.kernParam.d_forceParams, (void*)h_bytes, sizeof(t_iparams) * test.numFFParamTypes, cudaMemcpyHostToDevice) ); 
    ...
    for (int i=0; i < numFTypesOnGpu; i++) {
         ...
         MY_CUDA_CHECK( cudaMalloc((void**)&test.kernParam.d_iatoms[i], sizeof(t_iatom) * test.kernParam.numFTypeIAtoms[i]) ); 
         MY_CUDA_CHECK( cudaMemcpy((void*)test.kernParam.d_iatoms[i], (void*)h_iatoms, sizeof(t_iatom) * test.kernParam.numFTypeIAtoms[i], cudaMemcpyHostToDevice) ); 
    ...
    MY_CUDA_CHECK( cudaMalloc((void**)&test.kernParam.d_vTot, sizeof(float) * test.f_nre) ); 
    MY_CUDA_CHECK( cudaMemcpy((void*)test.kernParam.d_vTot, (void*)h_vTot, sizeof(float) * test.f_nre, cudaMemcpyHostToDevice) ); 
    ...
    MY_CUDA_CHECK( cudaMalloc((void**)&test.kernParam.d_xq, sizeof(float4) * test.natoms) ); 
    MY_CUDA_CHECK( cudaMemcpy((void*)test.kernParam.d_xq, h_xq, sizeof(float4) * test.natoms, cudaMemcpyHostToDevice) ); 
    ...
    MY_CUDA_CHECK( cudaMalloc((void**)&test.kernParam.d_f, sizeof(fvec) * test.natoms) ); 
    MY_CUDA_CHECK( cudaMemcpy((void*)test.kernParam.d_f, h_f, sizeof(fvec) * test.natoms, cudaMemcpyHostToDevice) ); 
    ...
    MY_CUDA_CHECK( cudaMalloc((void**)&test.kernParam.d_fShift, sizeof(fvec) * test.nshifts) ); 
    MY_CUDA_CHECK( cudaMemcpy((void*)test.kernParam.d_fShift, h_fShift, sizeof(fvec) * test.nshifts, cudaMemcpyHostToDevice) ); 
...
static void save_kernel_test_output(std::string dataDir, kernelTest &test, bool calcVir, bool calcEner)
{
    ...
    MY_CUDA_CHECK( cudaMemcpy((void*)h_vTot, (void*)test.kernParam.d_vTot, sizeof(float) * test.f_nre, cudaMemcpyDeviceToHost) ); 
    for (int i=0; i < F_NRE; i++) 
         ostr << h_vTot[i] << " "; 
    ostr << std::endl; 
    ...
    MY_CUDA_CHECK( cudaMemcpy((void*)h_f, (void*)test.kernParam.d_f, sizeof(fvec) * test.natoms, cudaMemcpyDeviceToHost) ); 
    for (int i=0; i < test.natoms; i++) 
    ...
    MY_CUDA_CHECK( cudaMemcpy((void*)h_fShift, (void*)test.kernParam.d_fShift, sizeof(fvec) * test.nshifts, cudaMemcpyDeviceToHost) ); 
    for (int i=0; i < test.nshifts; i++)   
...
{
    MY_CUDA_CHECK( cudaFree(test.kernParam.d_f) ); 
    MY_CUDA_CHECK( cudaFree(test.kernParam.d_vTot) ); 
    MY_CUDA_CHECK( cudaFree(test.kernParam.d_fShift) ); 
    MY_CUDA_CHECK( cudaFree((void*)test.kernParam.d_xq) ); 
    MY_CUDA_CHECK( cudaFree(test.kernParam.d_forceParams) ); 

    for (int i=0; i < numFTypesOnGpu; i++) {
         MY_CUDA_CHECK( cudaFree(test.kernParam.d_iatoms[i]) ); 
    }; 
}; 
...
int main(int argc, char *argv[])
{
    ...
    kernelFunc<<<dim3(test.gridSize[0],test.gridSize[1],test.gridSize[2]), dim3(test.blockSize[0],test.blockSize[1],test.blockSize[2]), 0, stream>>>(test.kernParam);  
