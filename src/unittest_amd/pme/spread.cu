...
int main(int argc, char *argv[]) {
    ...
    cudaMalloc((void**)&d_realGridSize, DIM * sizeof(int));
    cudaMalloc((void**)&d_realGridSizePadded, DIM * sizeof(int));
    cudaMalloc((void**)&d_tableOffsets, DIM * sizeof(int));

    cudaMalloc((void**)&d_recipBox, DIM * DIM * sizeof(float));
    cudaMalloc((void**)&d_realGridSizeFP, DIM * sizeof(float));

    cudaMalloc((void**)&d_theta, nAtomsPadded * DIM * order * sizeof(float));
    cudaMalloc((void**)&d_dtheta, nAtomsPadded * DIM * order * sizeof(float));
    cudaMalloc((void**)&d_gridlineIndices, nAtomsPadded * DIM * sizeof(int));

    cudaMalloc((void**)&d_coordinates, nAtomsPadded * DIM *sizeof(float));
    cudaMalloc((void**)&d_coefficients, nAtomsPadded * sizeof(float));
    cudaMalloc((void**)&d_fractShiftsTable, newFractShiftsSize * sizeof(float));

    cudaMalloc((void**)&d_gridlineIndicesTable, newFractShiftsSize * sizeof(int));

    float *d_realGrid;
    cudaMalloc((void**)&d_realGrid, nRealGridSize * sizeof(float));

    cudaMemcpy(d_realGridSize, h_realGridSize, DIM * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_realGridSizePadded, h_realGridSizePadded, DIM * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_tableOffsets, h_tableOffsets, DIM * sizeof(int), cudaMemcpyHostToDevice);

    cudaMemcpy(d_recipBox, h_recipBox, DIM * DIM * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_realGridSizeFP, h_realGridSizeFP, DIM * sizeof(float), cudaMemcpyHostToDevice);

    cudaMemcpy(d_theta, h_theta, nAtomsPadded * DIM * order * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_dtheta, h_dtheta, nAtomsPadded * DIM * order * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_gridlineIndices, h_gridlineIndices, nAtomsPadded * DIM * sizeof(int), cudaMemcpyHostToDevice);

    cudaMemcpy(d_coordinates, h_coordinates, nAtomsPadded * DIM * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_coefficients, h_coefficients, nAtomsPadded * sizeof(float), cudaMemcpyHostToDevice);

    cudaMemcpy(d_fractShiftsTable, h_fractShiftsTable, newFractShiftsSize * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_gridlineIndicesTable, h_gridlineIndicesTable, newFractShiftsSize * sizeof(int), cudaMemcpyHostToDevice);
    ...
    spreadKernel<<<dim3(grid_dim_x, 1, 1), dim3(order, useOrderThreads?1:order, atomPerBlock)>>> (true, true, true, true, nAtoms, d_coefficients, d_coordinates, d_realGridSizeFP, d_recipBox, d_fractShiftsTable, d_gridlineIndicesTable, d_tableOffsets, d_realGridSize, d_realGridSizePadded, d_theta, d_dtheta, d_gridlineIndices, d_realGrid);

    cudaDeviceSynchronize();

    //float *h_realGrid = new float[nRealGridSize];

    //cudaMemcpy(h_realGrid, d_realGrid, nRealGridSize * sizeof(float) , cudaMemcpyDeviceToHost);
    //cudaMemcpy(h_gridlineIndices, d_gridlineIndices, nAtoms * DIM * sizeof(float) , cudaMemcpyDeviceToHost);
    //cudaMemcpy(h_theta, d_theta, nAtoms * DIM * order * sizeof(float) , cudaMemcpyDeviceToHost);

    //for (int i = 0; i < nRealGridSize; i++)
    //    std::cout << h_realGrid[i] << std::endl;
    //for (int i = 0; i < nAtoms * DIM * order; i++)
    //    std::cout << h_theta[i] << std::endl;

    cudaFree(d_realGridSize);
    cudaFree(d_realGridSizePadded);
    cudaFree(d_tableOffsets);

    cudaFree(d_recipBox);
    cudaFree(d_realGridSizeFP);

    cudaFree(d_theta);
    cudaFree(d_dtheta);
    cudaFree(d_gridlineIndices);

    cudaFree(d_coordinates);
    cudaFree(d_coefficients);
    cudaFree(d_fractShiftsTable);

    cudaFree(d_gridlineIndicesTable);
