...
typedef struct cu_atomdata
{
    ...
    int4* xq; /**< atom coordinates + charges, size natoms      */
    int3* f;  /**< force output array, size natoms              */

    float* e_lj; /**< LJ energy output, size 1                     */
    float* e_el; /**< Electrostatics energy input, size 1          */

    int3* fshift; /**< shift forces                                 */

    int     ntypes;     /**< number of atom types                         */
    int*    atom_types; /**< atom type indices, size natoms               */
    int2* lj_comb;    /**< sqrt(c6),sqrt(c12) size natoms               */

    int3* shift_vec;         /**< shifts                                       */
...
__forceinline__ __host__ __device__ float norm2(int3 a)
{
...
static __forceinline__ __device__ void
                       reduce_force_j_warp_shfl(int3 f, int3* fout, int tidxi, int aidx)
{
...
static __forceinline__ __device__ void reduce_force_i_warp_shfl(int3             fin,
                                                                int3*            fout,
                                                                float*             fshift_buf,
                                                                bool               bCalcFshift,
                                                                int                tidxj,
...
static __forceinline__ __device__ void
                       reduce_energy_warp_shfl(int E_lj, int E_el, int* e_lj, int* e_el, int tidx, const unsigned long activemask)
{
    int i, sh;
...
#    else
    const int2* lj_comb = atdat.lj_comb;
    int2        ljcp_i, ljcp_j;
#    endif
    const int4*        xq          = atdat.xq;
    int3*              f           = atdat.f;
    const int3*        shift_vec   = atdat.shift_vec;
...
#    endif
    ...
    int4       xqbuf;
    int3       xi, xj, rv, f_ij, fcj_buf;
    int3       fci_buf[c_numClPerSupercl]; /* i force buffer */
    nbnxn_sci_t  nb_sci;
...
#    else
    ...
    int2* ljcpib = (int2*)sm_nextSlotPtr;
...
#    endif
    ...
    if (tidxz == 0)
    {
        ...
        xqbuf = xq[ai] + make_int4(LDG(shiftptr), LDG(shiftptr + 1), LDG(shiftptr + 2), 0.0f);
...
//            __syncwarp(c_fullWarpMask); //cm todo
            ...
            for (jm = 0; jm < c_nbnxnGpuJgroupSize; jm++)
            {
                if (imask & (superClInteractionMask << (jm * c_numClPerSupercl)))
                {
                    ...
                    /* load j atom data */
                    xqbuf = xq[aj];
                    xj    = make_int3(xqbuf.x, xqbuf.y, xqbuf.z);
                    qj_f  = xqbuf.w;
#    ifndef LJ_COMB
                    typej = atom_types[aj];
#    else
...
#    endif
                    for (i = 0; i < c_numClPerSupercl; i++)
                    {
                        if (imask & mask_ji)
                        {
                            ...
                            xi    = make_int3(xqbuf.x, xqbuf.y, xqbuf.z);
...
#    ifdef CALC_ENERGIES
    ...
    reduce_energy_warp_shfl(E_lj, E_el, (int*)e_lj, (int*)e_el, tidx, c_fullWarpMask);
#    endif
}
...
void copy_xq_to_gpu(cu_atomdata_t *adat) {
    ...
    hipMalloc((void**)&adat->xq, natoms * sizeof(int4));
...
void copy_lj_comb_to_gpu(cu_atomdata_t *adat) {
    ...
    hipMalloc((void**)&adat->lj_comb, natoms * sizeof(int2));
    hipMemcpyHtoD(adat->lj_comb, lj_comb, natoms * sizeof(int2));
...
void copy_shift_vec_to_gpu(cu_atomdata_t *adat) {
    ...
    hipMalloc((void**)&adat->shift_vec, natoms * sizeof(int3));
    hipMemcpyHtoD(adat->shift_vec, shift_vec, natoms * sizeof(int3));
                                                ...
                                                const cu_nbparam_t*     nbp)
{
    ...
    shmem = c_numClPerSupercl * c_clSize * sizeof(int4);
    ...
    if (nbp->vdwtype == evdwCuCUTCOMBGEOM || nbp->vdwtype == evdwCuCUTCOMBLB)
    {
        ...
        shmem += c_numClPerSupercl * c_clSize * sizeof(int2);
    }
    else
    {
...
int main (int argc, char* argv[]) {
    ...
    hipMalloc((void**)&adat->f, adat->natoms * sizeof(int3));
    hipMalloc((void**)&adat->fshift, adat->natoms * sizeof(int3));
