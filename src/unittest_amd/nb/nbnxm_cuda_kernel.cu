...
static const unsigned long c_fullWarpMask = 0xffffffff;
...
void copy_xq_to_gpu(cu_atomdata_t *adat) {
    ...
    cudaMalloc((void**)&adat->xq, natoms * sizeof(float4));
    cudaMemcpy(adat->xq, xq, natoms * sizeof(*adat->xq), cudaMemcpyHostToDevice);
...
void dump_xq(cu_atomdata_t *adat) {
    ...
    float *xq   = new float[natoms*4];
    cudaMemcpy(xq, adat->xq, natoms * sizeof(*adat->xq), cudaMemcpyDeviceToHost);
...
void copy_energy_to_gpu(cu_atomdata_t *adat) {
    ...
    cudaMalloc((void**)&adat->e_lj, sizeof(float));
    cudaMalloc((void**)&adat->e_el, sizeof(float));
    cudaMemcpy(adat->e_lj, e_lj, sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(adat->e_el, e_el, sizeof(float), cudaMemcpyHostToDevice);
...
void copy_atom_types_to_gpu(cu_atomdata_t *adat) {
    ...
    cudaMalloc((void**)&adat->atom_types, natoms * sizeof(int));
    cudaMemcpy(adat->atom_types, atom_types, natoms * sizeof(int), cudaMemcpyHostToDevice);
...
void copy_lj_comb_to_gpu(cu_atomdata_t *adat) {
    ...
    cudaMalloc((void**)&adat->lj_comb, natoms * sizeof(float2));
    cudaMemcpy(adat->lj_comb, lj_comb, natoms * sizeof(float2), cudaMemcpyHostToDevice);
...
void copy_shift_vec_to_gpu(cu_atomdata_t *adat) {
    ...
    cudaMalloc((void**)&adat->shift_vec, natoms * sizeof(float3));
    cudaMemcpy(adat->shift_vec, shift_vec, natoms * sizeof(float3), cudaMemcpyHostToDevice);
...
void copy_sci_to_gpu(cu_plist_t *plist) {
    ...
    cudaMalloc((void**)&plist->sci, nsci * sizeof(nbnxn_sci_t));
    cudaMemcpy(plist->sci, sci, nsci * sizeof(*plist->sci), cudaMemcpyHostToDevice);
...
void dump_sci(cu_plist_t *plist) {
    ...
    cudaMemcpy(sci, plist->sci, nsci * sizeof(*plist->sci), cudaMemcpyDeviceToHost);
...
void copy_cj4_to_gpu(cu_plist_t *plist) {
    ...
    cudaMalloc((void**)&plist->cj4, ncj4 * sizeof(nbnxn_cj4_t));
    cudaMemcpy(plist->cj4, cj4, ncj4 * sizeof(*plist->cj4), cudaMemcpyHostToDevice);
...
void copy_excl_to_gpu(cu_plist_t *plist) {
    ...
    cudaMalloc((void**)&plist->excl, nexcl * sizeof(nbnxn_excl_t));
    cudaMemcpy(plist->excl, excl, nexcl * sizeof(*plist->excl), cudaMemcpyHostToDevice);
...
void init_nbparam(cu_nbparam_t *nbp, int nbfpSize) {
    ...
    cudaMalloc((void**)&nbp->nbfp,  nbfpSize* sizeof(float));
    cudaMemcpy(nbp->nbfp, nbfp, nbfpSize * sizeof(float), cudaMemcpyHostToDevice);
...
void dump_nbparam(cu_nbparam_t *nbp, int nbfpSize) {
    ...
    float *nbfp  = new float[nbfpSize];
    cudaMemcpy(nbfp, nbp->nbfp, nbfpSize * sizeof(*nbp->nbfp), cudaMemcpyDeviceToHost);
...
int main (int argc, char* argv[]) {
    ...
    cudaMalloc((void**)&adat->f, adat->natoms * sizeof(float3));
    cudaMalloc((void**)&adat->fshift, adat->natoms * sizeof(float3));
    ...
    nbnxn_kernel<<<dim3(plist->nsci, 1, 1),dim3(c_clSize, c_clSize, num_threads_z),sharedMemorySize,0>>> (*adat, *nbparam, *plist, bCalcFshift);

    cudaFree(adat->xq);
    cudaFree(adat->e_lj);
    cudaFree(adat->e_el);
    cudaFree(adat->atom_types);
    cudaFree(adat->lj_comb);
    cudaFree(adat->shift_vec);
    cudaFree(plist->sci);
    cudaFree(plist->cj4);
    cudaFree(plist->excl);
    cudaFree(nbparam->nbfp);
    cudaFree(adat->f);
    cudaFree(adat->fshift);
