...
/* Fw. decl. */
static void nbnxn_cuda_clear_e_fshift(gmx_nbnxn_hip_t* nb);
...
static void cuda_init_const(gmx_nbnxn_hip_t*               nb,
                            const interaction_const_t*      ic,
                            const PairlistParams&           listParams,
                            const nbnxn_atomdata_t::Params& nbatParams)
{
...
gmx_nbnxn_hip_t* gpu_init(const gmx_device_info_t*   deviceInfo,
                           const interaction_const_t* ic,
                           const PairlistParams&      listParams,
...
void gpu_init_pairlist(gmx_nbnxn_hip_t* nb, const NbnxnPairlistGpu* h_plist, const InteractionLocality iloc)
{
    char         sbuf[STRLEN];
...
void gpu_upload_shiftvec(gmx_nbnxn_hip_t* nb, const nbnxn_atomdata_t* nbatom)
{
...
static void nbnxn_cuda_clear_f(gmx_nbnxn_hip_t* nb, int natoms_clear)
{
    hipError_t    stat;
    cu_atomdata_t* adat = nb->atdat;
    hipStream_t   ls   = nb->stream[InteractionLocality::Local];
...
void gpu_clear_outputs(gmx_nbnxn_hip_t* nb, bool computeVirial)
{
...
void gpu_init_atomdata(gmx_nbnxn_hip_t* nb, const nbnxn_atomdata_t* nbat)
{
    hipError_t    stat;
    int            nalloc, natoms;
    bool           realloced;
...
void gpu_free(gmx_nbnxn_hip_t* nb)
{
    hipError_t    stat;
    cu_atomdata_t* atdat;
...
gmx_wallclock_gpu_nbnxn_t* gpu_get_timings(gmx_nbnxn_hip_t* nb)
{
...
int gpu_min_ci_balanced(gmx_nbnxn_hip_t* nb)
{
    return nb != nullptr ? gpu_min_ci_balanced_factor * nb->dev_info->prop.multiProcessorCount : 0;
}

gmx_bool gpu_is_kernel_ewald_analytical(const gmx_nbnxn_hip_t* nb)
{
