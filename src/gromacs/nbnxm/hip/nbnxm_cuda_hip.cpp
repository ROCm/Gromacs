    ...
    */
    launchGpuKernel(kernel, config, deviceStream, timingEvent, "k_calc_nb", *adat, *nbp, *plist, stepWork.computeVirial);
        ...
        if ( sumUpEnergy || sumUpShifts )
        {
            ...
            configSumUp.gridSize[0]  = sumUpShifts ? SHIFTS : 1;
...
void gpu_launch_kernel_pruneonly(NbnxmGpu* nb, const InteractionLocality iloc, const int numParts)
{
     ...
     * - The 1D block-grid contains as many blocks as super-clusters.
     */
    int num_threads_z = plist->haveFreshList ? c_cudaPruneKernelJ4Concurrency / 2 : c_cudaPruneKernelJ4Concurrency;
    ...
    const auto     kernel =
	    plist->haveFreshList ? nbnxn_kernel_prune_cuda<true, c_cudaPruneKernelJ4Concurrency / 2> :
                                   nbnxn_kernel_prune_cuda<false, c_cudaPruneKernelJ4Concurrency>;
    /*
