...
#    endif
     ...
     * The loop stride NTHREAD_Z ensures that consecutive sub-groups are assigned
...
#    pragma unroll 8
                    for (int i = 0; i < c_numClPerSupercl; i++)
                    {
                        if (imaskCheck & mask_ji)
                        {
                            ...
                            //if (haveFreshList && !__any_sync(c_fullWarpMask, r2 < rlistOuter_sq)) //cm todo
                            ...
                            //if (__any_sync(c_fullWarpMask, r2 < rlistInner_sq)) //cm todo
