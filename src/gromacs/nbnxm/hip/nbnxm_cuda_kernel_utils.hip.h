...
*/
    ...
    fin.x += __shfl_down(fin.x, 4*c_clSize); //review
                       ...
                       reduce_energy_warp_shfl(float E_lj, float E_el, float* e_lj, float* e_el, int tidx, const unsigned int activemask)
{
    ...
    sh = 1;
#    pragma unroll warp_size_log2 //review
    for (i = 0; i < warp_size_log2; i++) //review
    {
/*
        E_lj += __shfl_down_sync(activemask, E_lj, sh);
        E_el += __shfl_down_sync(activemask, E_el, sh); //cm todo
*/
