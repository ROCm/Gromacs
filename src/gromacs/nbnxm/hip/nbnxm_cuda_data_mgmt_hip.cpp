...
static void init_atomdata_first(cu_atomdata_t* ad, int ntypes, const DeviceContext& deviceContext)
{
    ...
    allocateDeviceBuffer(&ad->fshift, (c_clShiftMemoryMultiplier + 1) * SHIFTS, deviceContext);
    allocateDeviceBuffer(&ad->e_lj, c_clEnergyMemoryMultiplier + 1, deviceContext);
    allocateDeviceBuffer(&ad->e_el, c_clEnergyMemoryMultiplier + 1, deviceContext);
...
static void nbnxn_cuda_clear_e_fshift(NbnxmGpu* nb)
{
    ...
    clearDeviceBufferAsync(&adat->fshift, 0, (c_clShiftMemoryMultiplier + 1) * SHIFTS, localStream);
    clearDeviceBufferAsync(&adat->e_lj, 0, c_clEnergyMemoryMultiplier + 1, localStream);
    clearDeviceBufferAsync(&adat->e_el, 0, c_clEnergyMemoryMultiplier + 1, localStream);
}
