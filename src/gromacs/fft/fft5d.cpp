                         ...
                         gmx::PinningPolicy realGridAllocationPinningPolicy)
{
    ...
    int N[3], M[3], K[3], pN[3], pM[3], pK[3], oM[3], oK[3],
            *iNin[3] = { nullptr }, *oNin[3] = { nullptr }, *iNout[3] = { nullptr },
            *oNout[3] = { nullptr };
    int        C[3], rC[3], nP[2];
    int        lsize;
...
#define EVENDIST
    ...
    if (!(flags & FFT5D_NOMALLOC))
    {
        ...
        if (realGridAllocationPinningPolicy == gmx::PinningPolicy::PinnedIfSupported && (GMX_GPU == GMX_GPU_CUDA || GMX_GPU == GMX_GPU_ROCM))
        {
    ...
    if ((!(flags & FFT5D_INPLACE)) && (!(P[0] > 1 || P[1] > 1))
        && nthreads == 1) /*don't do 3d plan in parallel or if in_place requested */
    {
...
#pragma omp ordered
    ...
    for (s = 0; s < 3; s++)
    {
        ...
        plan->iNin[s]  = iNin[s];
        plan->oNin[s]  = oNin[s];
        plan->iNout[s] = iNout[s];
        plan->oNout[s] = oNout[s];
    }
    for (s = 0; s < 2; s++)
    {
        plan->P[s]    = nP[s];
...
static void splitaxes(t_complex*       lout,
                      const t_complex* lin,
                      int              maxN,
                      int              maxM,
                      int              maxK,
...
/*transpose mayor and major dimension
   ...
   KG global size*/
static void joinAxesTrans13(t_complex*       lout,
                            const t_complex* lin,
                            int              maxN,
                            int              maxM,
                            int              maxK,
...
/*make axis contiguous again (after AllToAll) and also do local transpose
   ...
   N,M,K local size
   MG, global size*/
static void joinAxesTrans12(t_complex*       lout,
                            const t_complex* lin,
                            int              maxN,
                            int              maxM,
                            int              maxK,
...
static void compute_offsets(fft5d_plan plan, int xs[], int xl[], int xc[], int NG[], int s)
{
    ...
    if ((plan->flags & FFT5D_REALCOMPLEX)
        && ((!(plan->flags & FFT5D_BACKWARD) && s == 0) || ((plan->flags & FFT5D_BACKWARD) && s == 2)))
    {
        xl[0] = rC[s];
    }
}
...
void fft5d_execute(fft5d_plan plan, int thread, fft5d_time times)
{
    ...
    int *N = plan->N, *M = plan->M, *K = plan->K, *pN = plan->pN, *pM = plan->pM, *pK = plan->pK,
        *C = plan->C, *P = plan->P, **iNin = plan->iNin, **oNin = plan->oNin, **iNout = plan->iNout,
        **oNout = plan->oNout;
