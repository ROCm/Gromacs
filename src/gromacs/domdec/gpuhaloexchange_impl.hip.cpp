...
/*! \internal \file
 *
 * \brief Implements GPU halo exchange using HIP.
 *
 *
...
#include "gpuhaloexchange_impl.hpp"
...
void GpuHaloExchange::Impl::reinitHalo(float3* d_coordinatesBuffer, float3* d_forcesBuffer)
{
    ...
    GMX_RELEASE_ASSERT(cd.receiveInPlace,
                       "The HIP DD halo implementation only supports in place receive. "
...
#if GMX_MPI
    // Exchange of remote addresses from neighboring ranks is needed only with HIP-direct as hipmemcpy needs both src/dst pointer
                                                                        ...
                                                                        GpuEventSynchronizer* dependencyEvent)
{
    ...
    // ToDo: We need further refinement here as communicateHaloData includes launch time for hipmemcpyasync
