...
__launch_bounds__(c_spreadMaxThreadsPerBlock) CLANG_DISABLE_OPTIMIZATION_ATTRIBUTE __global__
        void pme_spline_and_spread_useorder_kernel(const PmeGpuHipKernelParams kernelParams)
{
    ...
    if (computeSplines)
    {
        ...
        calculate_splines_useorder<order, atomsPerBlock, atomsPerWarp, false, writeGlobal>(
...
pme_spline_and_spread_useorder_kernel<4, true, true, true, true, false, true>(const PmeGpuHipKernelParams);
