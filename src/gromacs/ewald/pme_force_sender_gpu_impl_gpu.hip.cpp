...
/*! \internal \file
 *
 * \brief Implememnts backend-specific code for PME-PP communication using HIP
 *
 *
...
void PmeForceSenderGpu::Impl::sendFToPpPeerToPeer(int ppRank, int numAtoms, bool sendForcesDirectToPpGpu)
{
    ...
    if (stageThreadMpiGpuCpuComm_ && !sendForcesDirectToPpGpu)
    {
        ...
        stat = hipMemcpyAsync(ppCommManagers_[ppRank].pmeRemoteCpuForcePtr,
                               ...
                               hipMemcpyDefault,
                               ppCommManagers_[ppRank].stream->stream());
        HIP_RET_ERR(stat, "hipMemcpyAsync on local device to host transfer of PME forces failed");
    }
