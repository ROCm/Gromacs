...
GpuParallel3dFft::GpuParallel3dFft(const PmeGpu* pmeGpu, const int gridIndex)
{
    ...
    realGrid_ = (hipfftReal*)kernelParamsPtr->grid.d_realGrid[gridIndex];
    GMX_RELEASE_ASSERT(realGrid_, "Bad (null) input real-space grid");
    complexGrid_ = (hipfftComplex*)kernelParamsPtr->grid.d_fourierGrid[gridIndex];
    ...
    configuration.stream=pmeGpu->archSpecific->pmeStream_.stream_pointer();
    ...
    hipStream_t stream = pmeGpu->archSpecific->pmeStream_.stream();
