 ...
 */
/*! \internal \file
 * \brief Defines the runner for ROCM version of SETTLE.
 *
...
{
    // These should never fail since this function should only be called if ROCM is enabled and
    // there is a ROCM-capable device available.
    GMX_RELEASE_ASSERT(GMX_GPU == GMX_GPU_ROCM,
                       "ROCM version of SETTLE was called from non-ROCM build.");

    // TODO: Here we should check that at least 1 suitable GPU is available
    GMX_RELEASE_ASSERT(canPerformGpuDetection(), "Can't detect ROCM-capable GPUs.");
