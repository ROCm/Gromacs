...
void pinBuffer(void* pointer, std::size_t numBytes) noexcept
{
    ...
    // (e.g. because HIP changes the set of errors it returns) then
    ...
    // engineer GROMACS to be forward-compatible with future HIP
    // versions, so if this proves to be a problem in practice, then
    // GROMACS must be patched, or a supported HIP version used.
...
void unpinBuffer(void* pointer) noexcept
{
    ...
    // future HIP changes the set of errors it returns) then we
