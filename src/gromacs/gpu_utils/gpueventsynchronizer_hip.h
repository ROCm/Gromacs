...
public:
    GpuEventSynchronizer()
    {
        ...
        GMX_RELEASE_ASSERT(stat == hipSuccess,
                           ("hipEventCreate failed. " + gmx::getDeviceErrorString(stat)).c_str());
    }
    ~GpuEventSynchronizer()
    {
        hipError_t gmx_used_in_debug stat = hipEventDestroy(event_);
        GMX_RELEASE_ASSERT(stat == hipSuccess,
                           ("hipEventDestroy failed. " + gmx::getDeviceErrorString(stat)).c_str());
    }
    //! No copying
    ...
    {
        hipError_t gmx_used_in_debug stat = hipEventRecord(event_, deviceStream.stream());
        GMX_ASSERT(stat == hipSuccess,
                   ("hipEventRecord failed. " + gmx::getDeviceErrorString(stat)).c_str());
    }
    ...
    inline void waitForEvent()
    {
        ...
        GMX_ASSERT(stat == hipSuccess,
                   ("hipEventSynchronize failed. " + gmx::getDeviceErrorString(stat)).c_str());
    }
    ...
    {
        hipError_t gmx_used_in_debug stat = hipStreamWaitEvent(deviceStream.stream(), event_, 0);
        GMX_ASSERT(stat == hipSuccess,
                   ("hipStreamWaitEvent failed. " + gmx::getDeviceErrorString(stat)).c_str());
    }
