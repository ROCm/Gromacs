    ...
    switch (transferKind)
    {
        case GpuApiCallBehavior::Async:
            ...
            stat = hipMemcpyAsync(*((ValueType**)buffer) + startingOffset, hostBuffer, bytes,
                                   hipMemcpyHostToDevice, deviceStream.stream());
            GMX_RELEASE_ASSERT(
                    stat == hipSuccess,
    ...
    switch (transferKind)
    {
        case GpuApiCallBehavior::Async:
            ...
            stat = hipMemcpyAsync(hostBuffer, *((ValueType**)buffer) + startingOffset, bytes,
                                   hipMemcpyDeviceToHost, deviceStream.stream());
            GMX_RELEASE_ASSERT(
                    stat == hipSuccess,
...
template<>
__forceinline__
void clearDeviceBufferAsync(DeviceBuffer<gmx::BasicVector<float>>* buffer,
                            size_t                                 startingOffset,
                            size_t                                 numValues,
                            const DeviceStream&                    deviceStream)
{
    ...
    auto kernelPtr            = kernel_fill<blockSize,itemsPerThread, gmx::BasicVector<float>>;
    launchGpuKernel(kernelPtr, config, deviceStream, nullptr, "kernel_fill", *((gmx::BasicVector<float>**)buffer) + startingOffset, gmx::BasicVector<float>(0.f, 0.f, 0.f), numValues);
