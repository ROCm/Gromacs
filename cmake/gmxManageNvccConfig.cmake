...
# Manage HIP nvcc compilation configuration, try to be smart to ease the users'
# pain as much as possible:
# - use the HIP_HOST_COMPILER if defined by the user, otherwise
# - check if nvcc works with HIP_HOST_COMPILER and the generated nvcc and C++ flags
#
# - (advanced) variables set:
#   * HIP_HOST_COMPILER_OPTIONS    - the full host-compiler related option list passed to nvcc
#
# Note that from CMake 2.8.10 FindHIP defines HIP_HOST_COMPILER internally,
# so we won't set it ourselves, but hope that the module does a good job.

# glibc 2.23 changed string.h in a way that breaks HIP compilation in
...
# feature and performance of memcpy variants is unimportant for HIP
...
function(work_around_glibc_2_23)
    ...
    if(IS_GLIBC_2_23_OR_HIGHER)
        message(STATUS "Adding work-around for issue compiling HIP code with glibc 2.23 string.h")
        list(APPEND HIP_HOST_COMPILER_OPTIONS "-D_FORCE_INLINES")
        set(HIP_HOST_COMPILER_OPTIONS ${HIP_HOST_COMPILER_OPTIONS} PARENT_SCOPE)
    endif()
endfunction()

gmx_check_if_changed(HIP_HOST_COMPILER_CHANGED HIP_HOST_COMPILER)

# set up host compiler and its options
if(HIP_HOST_COMPILER_CHANGED)
    set(HIP_HOST_COMPILER_OPTIONS "")
    ...
    if(APPLE AND CMAKE_C_COMPILER_ID MATCHES "GNU")
        ...
        # it for all versions.
        list(APPEND HIP_HOST_COMPILER_OPTIONS "-D__STRICT_ANSI__")
    endif()

    work_around_glibc_2_23()

    set(HIP_HOST_COMPILER_OPTIONS "${HIP_HOST_COMPILER_OPTIONS}"
        CACHE STRING "Options for nvcc host compiler (do not edit!).")

    mark_as_advanced(HIP_HOST_COMPILER HIP_HOST_COMPILER_OPTIONS)
endif()
...
# HIP source file is built. We've implemented that for current
...
# bypass this check.
if((_hip_nvcc_executable_or_flags_changed OR HIP_HOST_COMPILER_CHANGED OR NOT GMX_NVCC_WORKS) AND NOT WIN32)
    message(STATUS "Check for working NVCC/C++ compiler combination with nvcc '${HIP_NVCC_EXECUTABLE}'")
    execute_process(COMMAND ${HIP_NVCC_EXECUTABLE} -ccbin ${HIP_HOST_COMPILER} -c ${HIP_NVCC_FLAGS} ${HIP_NVCC_FLAGS_${_build_type}} ${CMAKE_SOURCE_DIR}/cmake/TestHIP.cu
        RESULT_VARIABLE _hip_test_res
        OUTPUT_VARIABLE _hip_test_out
        ERROR_VARIABLE  _hip_test_err
        OUTPUT_STRIP_TRAILING_WHITESPACE)

    if(${_hip_test_res})
        message(STATUS "Check for working NVCC/C compiler combination - broken")
        message(STATUS "${HIP_NVCC_EXECUTABLE} standard output: '${_hip_test_out}'")
        message(STATUS "${HIP_NVCC_EXECUTABLE} standard error:  '${_hip_test_err}'")
        if(${_hip_test_err} MATCHES "nsupported")
            message(FATAL_ERROR "NVCC/C++ compiler combination does not seem to be supported. HIP frequently does not support the latest versions of the host compiler, so you might want to try an earlier C++ compiler version and make sure your HIP compiler and driver are as recent as possible. Set the GMX_NVCC_WORKS CMake cache variable to bypass this check if you know what you are doing.")
        else()
            message(FATAL_ERROR "HIP compiler does not seem to be functional. Set the GMX_NVCC_WORKS CMake cache variable to bypass this check if you know what you are doing.")
        endif()
    elseif(NOT GMX_HIP_TEST_COMPILER_QUIETLY)
...
function(gmx_add_nvcc_flag_if_supported _output_variable_name_to_append_to _flags_cache_variable_name)
    ...
    if (NOT ${_flags_cache_variable_name} AND NOT WIN32)
        ...
        execute_process(
            COMMAND ${HIP_NVCC_EXECUTABLE} ${ARGN} "${CMAKE_SOURCE_DIR}/cmake/TestHIP.cu"
...
# If any of these manual override variables for target HIP GPU architectures
...
# architecture codes.
set(GMX_HIP_NVCC_GENCODE_FLAGS)
if (GMX_HIP_TARGET_SM OR GMX_HIP_TARGET_COMPUTE)
    set(_target_sm_list ${GMX_HIP_TARGET_SM})
    foreach(_target ${_target_sm_list})
        gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_AND_SM_${_target} -gencode arch=compute_${_target},code=sm_${_target})
        if (NOT NVCC_HAS_GENCODE_COMPUTE_AND_SM_${_target} AND NOT WIN32)
            message(FATAL_ERROR "Your choice of ${_target} in GMX_HIP_TARGET_SM was not accepted by nvcc, please choose a target that it accepts")
        endif()
    endforeach()
    set(_target_compute_list ${GMX_HIP_TARGET_COMPUTE})
    foreach(_target ${_target_compute_list})
        gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_${_target} -gencode arch=compute_${_target},code=compute_${_target})
        if (NOT NVCC_HAS_GENCODE_COMPUTE_${_target} AND NOT WIN32)
            message(FATAL_ERROR "Your choice of ${_target} in GMX_HIP_TARGET_COMPUTE was not accepted by nvcc, please choose a target that it accepts")
        endif()
    endforeach()
else()
    # Set the HIP GPU architectures to compile for:
    # - with HIP >=11.0        CC 8.0 is supported
    ...
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_AND_SM_35 -gencode arch=compute_35,code=sm_35)
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_AND_SM_37 -gencode arch=compute_37,code=sm_37)
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_AND_SM_50 -gencode arch=compute_50,code=sm_50)
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_AND_SM_52 -gencode arch=compute_52,code=sm_52)
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_AND_SM_60 -gencode arch=compute_60,code=sm_60)
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_AND_SM_61 -gencode arch=compute_61,code=sm_61)
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_AND_SM_70 -gencode arch=compute_70,code=sm_70)
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_AND_SM_75 -gencode arch=compute_75,code=sm_75)
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_AND_SM_80 -gencode arch=compute_80,code=sm_80)
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_AND_SM_86 -gencode arch=compute_86,code=sm_86)
    # Requesting sm or compute 35, 37, or 50 triggers deprecation messages with
    # nvcc 11.0, which we need to suppress for use in CI
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_WARNING_NO_DEPRECATED_GPU_TARGETS -Wno-deprecated-gpu-targets)
    ...
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_53 -gencode arch=compute_53,code=sm_53)
    gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_GENCODE_FLAGS NVCC_HAS_GENCODE_COMPUTE_80 -gencode arch=compute_80,code=sm_80)
endif()
...
if (GMX_HIP_TARGET_COMPUTE)
    set_property(CACHE GMX_HIP_TARGET_COMPUTE PROPERTY HELPSTRING "List of HIP virtual architecture codes to compile for (without the compute_ prefix)")
    set_property(CACHE GMX_HIP_TARGET_COMPUTE PROPERTY TYPE STRING)
endif()

# FindHIP.cmake is unaware of the mechanism used by cmake to embed
...
if (CMAKE_CXX_COMPILER_ID MATCHES "GNU" AND CMAKE_CXX_COMPILER_VERSION VERSION_LESS 8)
    list(APPEND GMX_HIP_NVCC_FLAGS "${CMAKE_CXX14_STANDARD_COMPILE_OPTION}")
else()
    list(APPEND GMX_HIP_NVCC_FLAGS "${CMAKE_CXX17_STANDARD_COMPILE_OPTION}")
endif()

# assemble the HIP flags
list(APPEND GMX_HIP_NVCC_FLAGS "${GMX_HIP_NVCC_GENCODE_FLAGS}")
gmx_add_nvcc_flag_if_supported(GMX_HIP_NVCC_FLAGS NVCC_HAS_USE_FAST_MATH -use_fast_math)

# assemble the HIP host compiler flags
list(APPEND GMX_HIP_NVCC_FLAGS "${HIP_HOST_COMPILER_OPTIONS}")

if (CMAKE_CXX_COMPILER_ID MATCHES "Clang")
    # HIP header hip_runtime_api.h in at least HIP 10.1 uses 0
    ...
    # HIP header crt/math_functions.h in at least HIP 10.x and 11.1
    ...
    # HIP headers hip_runtime.h and channel_descriptor.h in at least
    # HIP 11.0 uses many C-style casts, which are ncessary for this
    ...
    foreach(_flag ${NVCC_CLANG_SUPPRESSIONS_CXXFLAGS})
        list(APPEND GMX_HIP_NVCC_FLAGS "-Xcompiler ${_flag}")
    endforeach()
endif()

string(TOUPPER "${CMAKE_BUILD_TYPE}" _build_type)
gmx_check_if_changed(_hip_nvcc_executable_or_flags_changed HIP_NVCC_EXECUTABLE HIP_NVCC_FLAGS HIP_NVCC_FLAGS_${_build_type})
...
macro(GMX_SET_HIP_NVCC_FLAGS)
    set(HIP_NVCC_FLAGS "${GMX_HIP_NVCC_FLAGS};${HIP_NVCC_FLAGS}")
endmacro()
...
# hip_add_library(). This is the only way hip_add_library() can
...
function(gmx_hip_add_library TARGET)
    ...
    gmx_hip_target_compile_options(HIP_${TARGET}_CXXFLAGS)
    list(APPEND CMAKE_CXX_FLAGS ${HIP_${TARGET}_CXXFLAGS})
    foreach(build_type ${build_types_with_explicit_flags})
        list(APPEND CMAKE_CXX_FLAGS_${build_type} ${HIP_${TARGET}_CXXFLAGS_${build_type}})
    endforeach()

    hip_add_library(${TARGET} ${ARGN})
endfunction()
