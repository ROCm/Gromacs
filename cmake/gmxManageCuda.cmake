...
# The earliest version of the HIP toolkit that supports c++17 is 11.0
set(REQUIRED_HIP_VERSION 11.0)
set(REQUIRED_HIP_COMPUTE_CAPABILITY 3.0)

set(GMX_GPU_HIP ON)

option(GMX_CLANG_HIP "Use clang for HIP" OFF)

if(GMX_DOUBLE)
    message(FATAL_ERROR "HIP acceleration is not available in double precision")
endif()

find_package(HIP ${REQUIRED_HIP_VERSION} REQUIRED)

# Try to execute ${HIP_NVCC_EXECUTABLE} --version and set the output
...
macro(get_hip_compiler_info COMPILER_INFO DEVICE_COMPILER_FLAGS HOST_COMPILER_FLAGS)
    if(NOT GMX_CLANG_HIP)
        if(HIP_NVCC_EXECUTABLE)
            ...
            execute_process(COMMAND ${HIP_NVCC_EXECUTABLE} --version
            ...
            if (${_nvcc_version_res} EQUAL 0)
                ...
                SET(${COMPILER_INFO} "${HIP_NVCC_EXECUTABLE} ${_nvcc_info_singleline}")
                string(TOUPPER ${CMAKE_BUILD_TYPE} _build_type)
                SET(_compiler_flags "${HIP_NVCC_FLAGS_${_build_type}}")
                if(HIP_PROPAGATE_HOST_FLAGS)
                    set(${HOST_COMPILER_FLAGS} BUILD_CXXFLAGS)
                else()
                    set(${HOST_COMPILER_FLAGS} "")
                endif()
                SET(${DEVICE_COMPILER_FLAGS} "${HIP_NVCC_FLAGS}${HIP_NVCC_FLAGS_${_build_type}}")
            else()
        ...
        # CXX compiler is the HIP compiler
        set(${COMPILER_INFO} "${CMAKE_CXX_COMPILER}  ${CMAKE_CXX_COMPILER_ID} ${CMAKE_CXX_COMPILER_VERSION}")
        # there are some extra flags
        set(${COMPILER_FLAGS} "${CMAKE_CXX_FLAGS} ${CMAKE_CXX_FLAGS_${_build_type}} ${GMX_HIP_CLANG_FLAGS}")
    endif()
endmacro ()

if(GMX_CLANG_HIP)
    include(gmxManageClangHipConfig)
    list(APPEND GMX_EXTRA_LIBRARIES ${GMX_HIP_CLANG_LINK_LIBS})
    link_directories("${GMX_HIP_CLANG_LINK_DIRS}")
else()
    # Using NVIDIA compiler
    if(NOT HIP_NVCC_EXECUTABLE)
        message(FATAL_ERROR "nvcc is required for a HIP build, please set HIP_TOOLKIT_ROOT_DIR appropriately")
    endif()
    # set up nvcc options
